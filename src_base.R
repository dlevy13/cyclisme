...
creat_fic=function(){
  ...
  if (length(liste !=0)){ 
    for (k in 1:length(liste)) {
      ...
      names(a)=c("type","date","dist","calor","duree","fc_moy","fc_max","vm","dplus","cadence","puissance","TSS")
    ...
    #cr?ation des var heures et minutes
    ...
    #en r semaine commence le dimanche (jour=1) -> réaffecter le dimanche ? la semaine pr?c?dente
    ...
    velo$cadence=ifelse(velo$type=="Course à pied",NA,velo$cadence)
    ## sauvegarde 
...
base_stat=function(filtre_an){
  ...
  Getstat=function(per){
    ...
    stat1=velo %>% 
      group_by(annee,periode) %>% summarise(
        ...
        minutes=sum(minutes,na.rm=T),TSS=sum(TSS,na.rm=T))
  ...
  ## pour la puissance moyenne pond?r?e donc filtre supp
  ...
  statMois=Getstat("mois") %>% rename("dplus_M"=dplus,"dist_M"=dist,"heures_M"=heures,
                                      "minutes_M"=minutes,"TSS_M"=TSS) %>% 
  ...
  for (an in unique(velo$annee)) {
    ...
    #rep?rer les semaines sur 2 mois
  ...
  statSem=statSem %>% select(mois,semaine,annee,dplus,dist,duree,tps,puis,cad,TSS,
                             dplus_M,dist_M,duree_M,tps_M,puis_M,cad_M,TSS_M,heures,minutes)
  ...
  statSem=statSem %>% inner_join(cumul %>% select(semaine,annee,Cumdist,Cumduree,Cumtps,Cumdplus,CumTSS),
                                 by=c("annee","semaine"))
...
getCourbeCum=function(df,sem,donn){
  ...
  #pour année antérieur on prend le max de toute l'annéé
  s22=statSem %>% filter(annee==2022) %>% summarise(sem=max(semaine))
  s21=statSem %>% filter(annee==2021) %>% summarise(sem=max(semaine))
  
  m23=round(max(bbas[bbas$annee=="2023" & bbas$semaine<=m,]$var),0)
  m22=round(max(bbas[bbas$annee=="2022" & bbas$semaine==s22$sem,]$var),0)
  m21=round(max(bbas[bbas$annee=="2021" & bbas$semaine==s21$sem,]$var),0)
  
  if(donn == "Cumtps"){
    m23lib=max(bbas[bbas$annee=="2023" & bbas$semaine==m,]$Cumduree)
    m22lib=max(bbas[bbas$annee=="2022" & bbas$semaine==s22$sem,]$Cumduree)
    m21lib=max(bbas[bbas$annee=="2021" & bbas$semaine==s21$sem,]$Cumduree)
  }
  if(donn == "Cumdist"){
  ...
  if(donn == "CumTSS"){
    m23lib=paste0(format(m23, big.mark=" "),"")
    m22lib=paste0(format(m22, big.mark=" "),"")
    m21lib=paste0(format(m21, big.mark=" "),"")
  }
...
creat_ficNolio=function(){
  ...
  if(length(liste)!=0) {
    ...
    print("Aucun fichier de donn?es nouveau")
  }
}
